...
/** Reads `harness.canvas` from .claude/harness.config.json if the project has one; falls back to
 *  the same literal defaults `templates/expo.json` ships, so the command works with zero config. */
function canvasConfig() {
  try {
    return load().config?.harness?.canvas ?? {};
  } catch (e) {
